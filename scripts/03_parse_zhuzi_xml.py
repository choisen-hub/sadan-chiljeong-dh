  ...
  한 줄에 한 권. 각 레코드 구조:
    {
      "juan_num": 1,
      "juan_label": "卷一",
      "source": "zhuzi_tei",           # 02번은 file_name, 03번은 source 고정 문자열
      "headings": [
        {"level": 2, "text": "理氣上"},
        {"level": 3, "text": "太極、天地上"},
        {"level": 4, "text": "太極"},
      ...
      "paragraphs": [...],
      "raw_concat": "..."              # 문단 전체 이어붙임 (표점 포함)
    }

처리 규칙:
  - <pb n="..."/> 페이지 구분 제거
  - <hi>, <byline>, <note> 등 inline 요소 내용은 유지
...
def extract_text(elem) -> str:
    ...
    <hi>, <byline> 등 인라인 요소의 텍스트는 정상 포함.
    """
    parts: list[str] = []
    if elem.text:
...
def collect_content(div, depth: int, headings: list[dict], paragraphs: list[str]) -> None:
    """
    div 요소를 재귀 탐색하면서 headings와 paragraphs를 누적한다.
    ...
    for subdiv in div.findall(q("div")):
        collect_content(subdiv, depth + 1, headings, paragraphs)
    ...
    for top_div in top_divs:
        ...
        if juan_label == "提要":
            ...
            for p in top_div.findall(q("p")):
                t = extract_text(p).strip()
                if t:
                    paragraphs.append(t)
            records.append(make_record("提要", headings, paragraphs))
            ...
            for sub in top_div.findall(q("div")):
                ...
                # "朱子語類原序" → "原序"로 정규화 (칸리포 라벨과 일치시키기 위해)
                sub_label = sub_label_raw.replace("朱子語類", "")

                headings, paragraphs = [], []
                collect_content(sub, 0, headings, paragraphs)
                records.append(make_record(sub_label, headings, paragraphs))
        else:
            # 일반 권
            headings, paragraphs = [], []
            collect_content(top_div, 0, headings, paragraphs)
            records.append(make_record(juan_label, headings, paragraphs))
...
def main() -> None:
    ...
    print(f"       총 글자:   {total_chars:,}  (표점 포함)")
    if failed:
