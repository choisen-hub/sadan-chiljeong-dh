...
표점 부여된 주자어류(03 산출물)를 sentence 단위로 분절한다.

이전 06_segment.py 의 후속. hanja.dev 일괄적용으로 표점 파이프라인이
일원화되면서 분절 입력도 03 의 paragraphs_punctuated 로 통일.

분절 단위 (3-level):
  - 권 (juan)              : 144 records (권0의 4섹션 포함)
...
전처리:
  ...
  정규식으로 한 번 더 제거. (02 가 처리했어야 하지만 누락 케이스 대비 안전망)

입력:
  data/intermediate/kanripo_punctuated.jsonl   (03)
...
def main() -> None:
    if not INPUT.exists():
        ...
        print("        먼저 03_punctuate.py 실행 필요", file=sys.stderr)
        sys.exit(1)
    ...
    for rec in records:
        ...
        paragraphs = rec.get("paragraphs_punctuated")
        if paragraphs is None:
            print(
                f"[ERROR] {juan_label}: paragraphs_punctuated 필드 없음. "
                f"03_punctuate.py 가 이 record 를 처리하지 못한 것. "
                f"03 을 다시 실행할 것.",
                file=sys.stderr,
            )
            sys.exit(1)
        bylines = rec.get("paragraphs_byline") or [None] * len(paragraphs)
